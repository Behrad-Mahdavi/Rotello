    ...
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-canvas px-4 py-8 transition-colors overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm sm:max-w-md rounded-3xl border-[1.5px] border-border bg-surface p-6 sm:p-8 shadow-[3.5px_3.5px_0_#202A5A] dark:shadow-[3.5px_3.5px_0_#59BBAF] transition-all">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-base sm:text-lg font-black text-default">ایجاد حساب کاربری</h2>
        </div>

        <form onSubmit={handleSignup} className="space-y-4" dir="rtl">
          <div>
            <label className="block text-xs sm:text-sm font-bold text-default mb-1.5">نام و نام خانوادگی</label>
            <div className="relative">
              ...
              />
              <User className="absolute right-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted pointer-events-none" />
            </div>
          </div>
          ...
          >
            ...
            <span>{loading ? 'در حال ثبت‌نام...' : 'ثبت‌نام در سامانه'}</span>
          </button>
        </form>

        <div className="mt-6 pt-4 border-t border-border/70 text-center text-xs sm:text-sm text-muted">
          قبلاً ثبت‌نام کرده‌اید؟{' '}
          <Link href="/login" className="font-bold text-action transition-colors hover:text-action-hover hover:underline">
            وارد شوید
          </Link>
        </div>
      </div>
